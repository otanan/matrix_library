...
Matrix *newMatrix(int m, int n) {
	...
	//is different
	double rowSize = self->__m__ * sizeof(double *);
	...
	for(int row = 1; row <= self->__m__; row++)
		self->__rows__[row - 1] = calloc(self->__n__, sizeof(double));
...
Matrix *IDENTITY(int dim) {
	...
	for(int i = 0; i < usedSpace; i++) {
		if(loadedIdentities[i]->m(loadedIdentities[i]) == dim)
...
double *__getRow__(Matrix *self, int row) {
	...
	return copyArray(self->__rows__[row - 1], self->n(self));
}
...
void __setRow__(Matrix *self, int row, double *a) {
	...
	self->__rows__[row - 1] = copyArray(a, self->n(self));
}
...
Matrix *matrixMult(Matrix *m1, Matrix *m2) {
	...
	for(int row = 1; row <= product->m(product); row++) {
		...
		for(int col = 1; col <= product->n(product); col++) {
			...
			Vector *m2ColVec = m2->getColVector(m2, col);
			product->setElem(product, row, col, dotProduct(m1RowVec, m2ColVec));

			m2ColVec->free(m2ColVec);
		}	
	}
